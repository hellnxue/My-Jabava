...
;(function(w, $){
	...
	window.jabava.socialAccumulationAccount['accountCities'] = window.jabava.socialAccumulationAccount['accountCities'] || {
		length:0
	};
	...
	.then(function(d){
		...
		var $el = {};
		window.jabava.socialAccumulationAccount.accountId = accountId;
		if( !Boolean( d.resultCode*1 ) ){
			$.each(d.resultData.dataList, function(index, item){
				$.each(item, function(key, val){
					$el = $form.find('[data-name='+key+']');

					if( key === 'socialSecurityCode' ){
						window.jabava.socialAccumulationAccount.securityCode = val;
					}
					if( key === 'accumulationFundCode' ){
						window.jabava.socialAccumulationAccount.accumulationCode = val;
					}
		...
		if( !Boolean(d.resultCode*1) ){
			$.each(d.resultData.dataList, function(index, item){
				$.each(item, function(key, val){
					...
					window.jabava.socialAccumulationAccount['accountCities'][key] = val;
					addTab( getParent );
						...
						$.each(d.resultData.dataList[0], function(key, val){
							...
							if( $el.length > 0 ){
								switch($el.prop('tagName').toLowerCase()){
									case 'select':
										if( key === 'provinceId' ) rulesFormAction.setCity($form, val);
										$el.val(val)
										.attr('data-current-val', val);
										$form.formValidation('resetField', $el);
										break;
									case 'input':
										if( $el.is('input:hidden') ){
											$el.val( getRuleId(val) );
											$(el)
											.find('[data-name='+key+']')
											.empty().append( renderList(val) );
										}
										break;
								}
							}else{
								$(el).find('.params-area')
								.prepend( $('<input type="hidden" name="'+key+'" value="'+val+'">') );
							}
						});
					}
				});
	...
	var rulesFormAction = {
		...
		setCity : function($form, provinceId){
			...
			$.get({
				url: 'area/getCitiesByProvinceId',
				dataType: 'json',
				async: false,
	...
	var loadRulesForm = function($container, SN){
		...
		var tplURI = 'static/tpls/socialaccumulation/editAccounts.hbt.js?_t='+unique();
		$.get({
			url : tplURI,
			async : false,
			dataType : 'html'
