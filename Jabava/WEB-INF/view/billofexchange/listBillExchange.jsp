                                ...
                                <form id="msForm" class="form-horizontal">
                                    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                                        <div class="form-group has-feedback">
                                            <label class="control-label col-xs-6 col-sm-6 col-md-6 col-lg-4">公积金账户：</label>
                                            <div class="col-xs-6 col-sm-6 col-md-6 col-lg-8">
                                                <select name="accumulationFundAccountId" id="accumulationFundAccountIdForSearch" class="form-control" >
                                                    <option value="">全部</option>
                                                    <c:forEach var="account" items="${requestScope.afAccountList }" varStatus="status">
                                                   		<option value="${account.accumulationFundAccountId }">${account.accumulationFundAccountName }</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                                         <div class="form-group">
                                            <label class="control-label col-xs-6 col-sm-6 col-md-6 col-lg-5">年月：</label>
                                            <div class="col-xs-6 col-sm-6 col-md-6 col-lg-7">
                                            <div class="input-group date "> 
                                                    <input type="text" id="monthForSearch" name="month" class="form-control">
                                    ...
                                    <div class="modal-header">
                                     ...
                                     <div class="panel-body">
                                      <form class="form-horizontal" id="addForm">
                                            ...
                                            <div class="form-group has-feedback">
                                                <label class="control-label col-xs-6 col-sm-6 col-md-6 col-lg-5">公积金账户：</label>
                                                <div class="col-xs-6 col-sm-6 col-md-6 col-lg-7">
                                                    <select name="accumulationFundAccountId" id="accumulationFundAccountId" class="form-control">
                                                        <option value="">全部</option>
	                                                    <c:forEach var="account" items="${requestScope.afAccountList }" varStatus="status">
	                                                   		<option value="${account.accumulationFundAccountId }">${account.accumulationFundAccountName }</option>
	                                                    </c:forEach>
...
<script type="text/javascript">
	...
	function loadTable(params){
	    ...
	    table = $('#projectTable').DataTable({
	        ...
	        "ordering": false,
	        "ajax": {
	            "url":"afPaymentBill/listPaymentBillPage",
	            "data": $('#msForm').serializeObject(),
	            "type":"post"
	        },
	        "columns": [
	             {"render": function render( data, type, row, meta ){
	                return'<input type="checkbox" name="checkAll" id="checkAll'+row.afPaymentBillId+'" value="'+row.afPaymentBillId+'" >';
	              },"orderable": false },
	            { "data": "accumulationFundAccountName","render": function render( data, type, row, meta ){
                    var strHtml = '<td>';
                    <% if(RequestUtil.hasPower("afbill_vb")){ %>
                    strHtml += '<a href="javascript:void(0);" onclick="viewDetail(' + row.afPaymentBillId + ')">' + data + '</a>';
                    <% }else{ %>
                    strHtml += data;
                    <% } %>
                    strHtml += '</td>';
	            ...
	            { "data": "status", "render": function(data, type, row, meta){
                    ...
                    return '<p id="status' + row.afPaymentBillId + '" data-status="' + data + '">' + text + '</p>';
	            } },
	            { "data": "amountE" },
	            { "data": "amountP" },
	            { "data": "hjNum" },
        ...
        strHtml += '<input type="hidden" name="afPaymentBillId"></form>';
        <% if(RequestUtil.hasPower("afbill_ex")){ %>
        strHtml += '<a href="report/commonGenReport?reportId=9" class="btn btn-info btn-sm" data-toggle="modal" data-target="[data-modal=confReport]">导出</a>&nbsp;';
        <% } %>
        ...
        $('[data-toggle="ascupload"]').on('click', function(event) {
            ...
            if( $getItems.length < 1 || $getItems.length > 1 ){
                ...
                $getAscuploadForm.find('[name="afPaymentBillId"]').val($getItems.val());
	...
	$('#addForm').formValidation({
	 ...
	 .on('success.form.fv', function(e){
	    e.preventDefault();
	    $.ajax({
			url:"afPaymentBill/generatePaymentBill",
			type : "POST",
			dataType:'json',
			data: {"accumulationFundAccountId": $('#accumulationFundAccountId').val(), "month": $('#month').val()},
	...
	function viewDetail(id){
	    $('#afPaymentBillId').val(id);
	    $('#detailForm').attr('action','afPaymentBill/toListPaymentBillPerson');
	    $('#detailForm').submit();
	}
	...
	function doOperate(type){
	    ...
	    $.ajax({
			url:"afPaymentBill/changeStatus",
			type : "POST",
			dataType:'json',
