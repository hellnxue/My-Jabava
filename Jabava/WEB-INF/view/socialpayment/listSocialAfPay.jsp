                        ...
                        <div class="panel-body m-t-md"> 
                            <table id="projectTable" class="table table-condensed table-hover table-bordered dataTable" width="100%">
                                <thead>
	                               <tr>
											<th rowspan="2">选择</th>
											<th rowspan="2">社保账户</th>
											<th rowspan="2">年月</th>
											<th colspan="15">社保汇缴</th>
											<th colspan="14">公积金汇缴</th>
											
										</tr>
										
										<tr>
										  
											<th style=" border-top-width: 1px;">处理状态</th>
											<th style=" border-top-width: 1px;">企业缴费总额</th>
											<th style=" border-top-width: 1px;">个人缴费总额</th>
											<th style=" border-top-width: 1px;">汇缴人数</th>
											<th style=" border-top-width: 1px;">企业汇缴基数</th>
											<th style=" border-top-width: 1px;">个人汇缴基数</th>
											<th style=" border-top-width: 1px;">企业汇缴总额</th>
											<th style=" border-top-width: 1px;">个人汇缴总额</th>
											<th style=" border-top-width: 1px;">汇缴总额</th>
											<th style=" border-top-width: 1px;">补缴人数</th>
											<th style=" border-top-width: 1px;">企业补缴基数</th>
											<th style=" border-top-width: 1px;">个人补缴基数</th>
											<th style=" border-top-width: 1px;">企业补缴总额</th>
											<th style=" border-top-width: 1px;">个人补缴总额</th>
											<th style=" border-top-width: 1px;">补缴总额</th>
...
<script type="text/javascript">
    ...
    function loadTable(params){
        ...
        table = $('#projectTable').DataTable({
            ...
            "ordering": false,
            "ajax": {
                "url":"ssafPaymentBill/listPaymentBillPage",
                "data": $('#msForm').serializeObject(),
                "type":"post"
            },
            "columns": [
                 {"render": function render( data, type, row, meta ){
                    return'<input type="checkbox" data-ss-id='+row.id+'  data-af-id='+row.aaf_payment_bill_id+' name="checkAll" id="checkAll'+row.id+'" value="'+row.id+'" >';
                  },"orderable": false },
                { "data": "social_security_account_name",
                    "render": function render( data, type, row, meta ){
                       	var strHtml = '<td>';
                       	var aaf_payment_bill_id;
                       	if(row.aaf_payment_bill_id==null){
                       		aaf_payment_bill_id=-1;
                       	}else{
                       		aaf_payment_bill_id=row.aaf_payment_bill_id;
                       	}
                        if(data==null){
                        	 <% if(RequestUtil.hasPower("asbill_vb")){ %>
                     			strHtml += '<a href="javascript:void(0);" onclick="viewDetail(' + row.id+','+aaf_payment_bill_id  + ')">' + row.aaaccumulation_fund_account_name + '</a>';
		                     <% }else{ %>
		                    	 strHtml += data;
		                     <% } %>
                        }else{
                        	 <% if(RequestUtil.hasPower("asbill_vb")){ %>
                     			strHtml += '<a href="javascript:void(0);" onclick="viewDetail(' + row.id+','+aaf_payment_bill_id  + ')">' + data + '</a>';
		                     <% }else{ %>
		                    	 strHtml += data;
		                     <% } %>
                        }
        ...
        <% if(RequestUtil.hasPower("asbill_ex")){ %>
        strHtml += '<a href="report/commonGenReport?reportId=20" class="btn btn-info btn-sm" data-toggle="modal" data-target="[data-modal=confReport]">导出</a>&nbsp;';
        <% } %>
    ...
    $('#addForm').formValidation({
     ...
     .on('success.form.fv', function(e){
        e.preventDefault();
        $.ajax({
			url:"ssafPaymentBill/generatePaymentBill",
			type : "POST",
			dataType:'json',
    ...
    function viewDetail(id,afPaymentBillId){
    	if(id){
    	...
    	if(afPaymentBillId){
    		 $('#afPaymentBillId').val(afPaymentBillId);
  	    }else{
  	    	 $('#afPaymentBillId').val("");
  	    }
    ...
    function doOperate(type){
        ...
        var ssId=$(":checked[name='checkAll']").data("ssId"),
            afId=$(":checked[name='checkAll']").data("afId");
        
        if(ssId !=undefined||afId !=undefined){
        	  $.ajax({
      			url:"ssafPaymentBill/changeStatus",
      			type : "POST",
      			dataType:'json',
      			data: {"afId": afId,"ssId": ssId, "operType": type},
      			error: function(XMLHttpRequest, textStatus, errorThrown){
      				alert(textStatus);
      			},
      			success: function(data){
