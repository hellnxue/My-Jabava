      ...
      <div class="panel-body">
        <div id="hbreadcrumb" class="m-t-xs m-b-xs">
          <h2 class="font-normal m-b-xs text-center">
            ${requestScope.ssPaymentBill.socialSecurityAccountName}${requestScope.ssPaymentBill.month}月清单明细-社保本月增员
          </h2>
        </div>
      </div>
    </div>
				...
				<div class="col-lg-12">
					<div class="hpanel">
						...
						<ul role="tabList" class="nav nav-tabs m-t-md" id="">
							...
							<li role="payDetails" class="active"><a href="#tab-1">本月增员</a>
							</li>
							<li role="payDetails"><a href="javascript: void(0);"
								onclick="viewPerson('JY');">本月减员</a></li>
							<li role="payDetails"><a href="javascript: void(0);"
								onclick="viewPerson('BG');">本月变更</a></li>
						</ul>
...
<script type="text/javascript">
	...
	var afPaymentBillId = '${requestScope.afPaymentBill.afPaymentBillId}';
    ...
    function loadHeader(){
    	$.ajax({
			url:"ssafPaymentBill/loadPaymentBillPersonZyHeader",
			type : "POST",
			dataType:'json',
    ...
    function initTable(columns){
    	detailTable = $('#detailTable').DataTable({
    		...
    		"ajax": {
    			"url": "ssafPaymentBill/listPaymentBillPersonZy",
    			"data": {
    ...
    function afLoadHeader(){
		$.ajax({
			url:"afPaymentBill/loadPaymentBillPersonZyHeader",
			type : "POST",
			dataType:'json',
			...
			success: function(data){
				if(data){
					...
					afInitDetailTable(data);
				}
			}
		});
	}
	
	function afInitDetailTable(data){
		//读取数据，初始化表头及DataTables的columns
		var columns = [];
		...
		header.append(firstRow);
		header.append(secondRow);
		$('#afDetailTable').append(header);
		
		afInitTable(columns);
	}
	
	function afInitTable(columns){
		afDetailTable = $('#afDetailTable').DataTable({
			"dom":
	    		//"<'row'<'col-sm-12'l>>" +
	    		"<'row'<'col-sm-12'tr>>",
	    		//"<'row'<'col-sm-5'i><'col-sm-7'p>>",
			...
			"ajax": {
				"url": "afPaymentBill/listPaymentBillPersonZy",
				"data": {
